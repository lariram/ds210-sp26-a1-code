...
impl ChatbotV4 {
    ...
    pub fn get_history(&self, username: String) -> Vec<String> {
        ...
        match file_library::load_chat_session_from_file(&filename) { // search for previous session/username
            None => {
                return Vec::new(); // if there is no previous chat session/the username has never been used before, return an empty history
            },
            Some(session) => {
